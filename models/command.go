		...
		if len(cks) == 0 {
			...
			for i := range cks {
				...
				if eachCk.WsKey == "" {
					...
					sender.Reply(fmt.Sprintf(`wskey="pin=%s;wskey=%s;" python3 wspt.py`, eachCk.PtPin, eachCk.WsKey))
					res := simpleCmd(fmt.Sprintf(`"pin=%s;wskey=%s;" python3 wspt.py`, eachCk.PtPin, eachCk.WsKey))
