...
type Yaml struct {
	...
	SmsToken            string `yaml:"SMS_Token"`
	SMSAddress          string `yaml:"SMS_Address"`
	Node                string
	Npm                 string
	Python              string
	Pip                 string
...
func initConfig() {
	...
	for _, name := range []string{"app.conf", "config.yaml", "reply.php"} {
		...
		if len(s) == 0 {
			...
			r, err := httplib.Get(GhProxy + "https://raw.githubusercontent.com/langhun/xdd/x/conf/demo_" + name).Response()
			if err == nil {
				io.Copy(f, r.Body)
			}
		}
